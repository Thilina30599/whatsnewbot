...
var r_text = new Array ();
    ...
    QueenSew.newcmdaddtosew({pattern: '18+ ?(.*)', fromMe: true, dontAdCommandList: true, desc: Mod }, (async (message, match) => {
   ...
   QueenSew.newcmdaddtosew({pattern: '18plus', fromMe: true, dontAdCommandList: true }, (async (message, match) => {
      await message.sendMessage('සෙව් Queen වට්සැප් බොට් මගින් මෙම සේවාව ලබා ගත නොහැකි අතර මෙම සේවාව ලබා ගැනීමට 18+ 18+ වට්සැප්.බොට් වෙත මාරු විය යුතුය... එම බොට් වෙතමාරු වීම සදහා\n\n .18+ කමාන්ඩ් එක හෝ \n .var BOT_MODE:18+ කමාන්ඩ් එක භාවිත කරන්න\n\n\n අවවාදයයි:  +18 බොට් තුල Sew Queen වට්සැප් බොට් තුල ඇති කිසිදු සේවාවක් ලබාගත නොහැකි අතර 18+mod යනු හුදෙක් විනෝදාස්වාදය සදහා නිපදවන ලද 18+ බොට් කෙනෙකි..')
   }));
if (Raviya.PSD !== ASS) {
...
if (Raviya.PSD == ASS) {
  if (Raviya.PSW == 'kingraviya') {
      QueenSew.newcmdaddtosew({pattern: 'thilina ?(.*)', fromMe: true, dontAdCommandList: true, desc: Sod }, (async (message, match) => {
...
QueenSew.newcmdaddtosew({pattern: 'pornhub ?(.*)', fromMe: true, desc: 'for pornhub search මෙයින් ලබා ගන්න ලින්ක් .xxx <link> මගින් බා ගන්න'}, (async (message, match) => { 
        ...
        await message.client.sendMessage(message.jid,'   _*👑ᴘᴏᴡᴇʀᴅ ʙʏ ᵀʰⁱˡⁱⁿᵃ ᴮᵒᵗ👑*_\n\n' + mesaj,MessageType.text);
        await reply.delete();
    }));
