...
if (config.WORKTYPE == 'private') {
    ...
    QueenSew.newcmdaddtosew({pattern: 'song ?(.*)', fromMe: true, desc: Lang.SONG_DESC}, (async (message, match) => { 
        ...
        ffmpeg(stream)
            ...
            .on('end', async () => {
                ...
                await message.client.sendMessage(message.jid,Buffer.from(writer.arrayBuffer), MessageType.document, {filename: 'thilina' + title + '.mp3', mimetype: 'audio/mpeg',quoted: message.data});
            });
    }));
    ...
    QueenSew.newcmdaddtosew({pattern: "covid ?(.*)", fromMe: false, desc: Clang.COV_DESC}, (async (message, match) => {
        ...
        else if (match[1] === "sl" || match[1] === "SL" || match[1] === "Sl" || match[1].includes('srilanka') ) {
            try{
